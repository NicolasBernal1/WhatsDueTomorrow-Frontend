...
describe('Protección de Rutas Privadas (Frontend) - Pruebas de Caminos Básicos', () => {
...
// =============================================================================
// PRUEBAS DE INTEGRACIÓN DE ENRUTAMIENTO (Angular Router Navigation Integration)
// =============================================================================
describe('Enrutamiento y Protección de Rutas - Integración con Router de Angular', () => {

  const testRoutes: Routes = [
    { path: 'login', component: MockPublicLoginComponent },
    { path: 'schedule', component: MockPrivateScheduleComponent, canActivate: [authGuard] },
  ...
  // Camino 1: [1, 2, 3, 8] - Navegación directa a ruta pública /login
  it('Camino 1: debe navegar exitosamente a /login sin requerir autenticación (Ruta pública)', async () => {
    authServiceMock.isAuthenticated.and.returnValue(false);
  ...
  // Camino 2: [1, 2, 4, 5, 6, 8] - Navegación a ruta privada /schedule con usuario autenticado
  it('Camino 2: debe navegar exitosamente a /schedule cuando el usuario tiene sesión activa (Ruta privada)', async () => {
    authServiceMock.isAuthenticated.and.returnValue(true);

    const navigationResult = await router.navigate(['/schedule']);

    expect(navigationResult).toBeTrue();
    expect(router.url).toBe('/schedule');
  });

  // Camino 3: [1, 2, 4, 5, 7, 8] - Intento de navegación a /schedule sin sesión activa redirige a /login
  it('Camino 3: debe bloquear navegación a /schedule y redirigir automáticamente a /login cuando no hay sesión activa', async () => {
    authServiceMock.isAuthenticated.and.returnValue(false);
